...
class OpenCVCamera(Camera):
    ...
    def read(self, color_mode: ColorMode | None = None) -> np.ndarray:
        ...
        # jiang resize到配置的分辨率
        if (processed_frame.shape[1], processed_frame.shape[0]) != (self.capture_width, self.capture_height):
            processed_frame = cv2.resize(processed_frame, (self.capture_width, self.capture_height))
        # jiang resize到配置的分辨率
