...
class ReplayBuffer:
    ...
    def _initialize_storage(
        ...
        self.tags = torch.empty((self.capacity,), dtype=torch.long, device=self.storage_device)
    ...
    def add(
        ...
        # #Jiang 
        # action_to_store = action[: self.actions.shape[1]]
        # self.actions[self.position].copy_(action_to_store.squeeze(dim=0))
    ...
    def sample(self, batch_size: int) -> BatchTransition:
        ...
        if hasattr(self, "tags"):
           batch_tags = self.tags[idx].to(self.device) 
        #*******
    ...
    def from_lerobot_dataset(
        ...
        if list_transition:
            ...
            # print(num_discrete_actions)
            # print(first_transition["action"])
            # print(first_transition["action"][:, : num_discrete_actions])
            # breakpoint()
            first_action = first_transition["action"].to(device)
            # first_action = first_transition["action"][:, : num_discrete_actions].to(device)
        ...
        for data in list_transition:
            ...
            # action = data["action"][:, : num_discrete_actions]
            action = data["action"]
            replay_buffer.add(
                state=data["state"],
                action=action,
...
def concatenate_batch_transitions(
    ...
    ####dhy
    left_batch_transitions["tag"] = torch.cat([left_batch_transitions["tag"], right_batch_transition["tag"]], dim=0)
