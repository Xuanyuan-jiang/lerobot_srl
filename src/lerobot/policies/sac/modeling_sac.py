...
class SACPolicy(
    ...
    def critic_forward(
        ...
        """
        # print("critic_forward called Actions shape:", actions.shape)
    ...
    def forward(
        ...
        if model == "temperature":
            loss_temperature, policy_entropy = self.compute_loss_temperature(
                observations=observations,
                observation_features=observation_features,
            )
            return {
                "loss_temperature": loss_temperature,        # Tensor
                "policy_entropy": policy_entropy.item(),     # float，方便日志
                "target_entropy": float(self.target_entropy) # 可选：一起返回，便于对比
            }
    ...
    def compute_loss_temperature(self, observations, observation_features: Tensor | None = None) -> Tensor:
        ...
        with torch.no_grad():
            _, log_probs, _ = self.actor(observations, observation_features)  #log_probs
        print("target_entropy", self.target_entropy)
        policy_entropy = (-log_probs).mean()  
        # breakpoint()
        temperature_loss = (-self.log_alpha.exp() * (log_probs + self.target_entropy)).mean()
        return temperature_loss, policy_entropy
...
class CriticEnsemble(nn.Module):
    ...
    def forward(
        ...
        # print(">> raw actions:", actions.shape)
        ...
        # print(">> normalized actions:", actions.shape)
        ...
        # print("concat:", inputs.shape)
        # print("critic[0].net:", self.critics[0].net)
        # breakpoint()
