...
from lerobot.robots.so101_follower import SO101FollowerEndEffectorConfig
...
class RobotEnv(gym.Env):
    ...
    def _get_observation(self) -> dict[str, np.ndarray]:
        ...
        for key in self._image_keys:
            img = obs_dict[key]  # (H, W, 3)
            img = cv2.resize(img, (128, 128), interpolation=cv2.INTER_LINEAR)         # (128, 128, 3)
            images[key] = img
...
class BaseLeaderControlWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if is_intervention:
            ...
            self._handle_leader_teleoperation() # leader跟随follower
    ...
    def _handle_key_press(self, key, keyboard_device):
        ...
        if key == keyboard_device.Key.space:
            if not self.keyboard_events["human_intervention_step"]:
                ...
                follower_pos_dict = {}
                # 排除不存在的 gripper
                for joint_name, pos in leader_pos_dict.items():
                    follower_pos_dict[joint_name] = pos
                    
                self.robot_follower.bus.sync_write("Goal_Position", follower_pos_dict)
            else:
                self.keyboard_events["human_intervention_step"] = False
