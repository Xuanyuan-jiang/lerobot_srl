...
def add_actor_information_and_train(
    ...
    while True:
        ...
        ###dhy 统计tag数量
        if "tag" in batch:
            tag_values = batch["tag"].cpu().numpy()
            tag_1_count = (tag_values == 1).sum()
            tag_0_count = (tag_values == 0).sum()
            print(f"Count of tag=1: {tag_1_count}, Count of tag=0: {tag_0_count}")
        ...
        if optimization_step % policy_update_freq == 0:
            for _ in range(policy_update_freq):
                ...
                training_infos["policy_entropy"] = temperature_output["policy_entropy"]       # ← 新增
                training_infos["target_entropy"] = temperature_output.get("target_entropy")   # ← 可选
...
def initialize_offline_replay_buffer(
    ...
    offline_replay_buffer = ReplayBuffer.from_lerobot_dataset(
        ...
        state_keys=cfg.policy.input_features.keys(),
        storage_device=storage_device,
        num_discrete_actions=cfg.policy.num_discrete_actions,
        optimize_memory=True,
...
def process_transitions(
    ...
    while not transition_queue.empty() and not shutdown_event.is_set():
        ...
        for transition in transition_list:
            ...
            # print("[LEARNER] Adding transition to replay buffer:", transition["action"].shape)
            transition.setdefault("tag", None)
            ...
            if dataset_repo_id is not None and transition.get("complementary_info", {}).get(
                ...
                # breakpoint()
                # print("[Intervention] Adding transition to replay buffer:", transition["action"])
